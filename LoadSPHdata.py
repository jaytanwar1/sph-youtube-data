...
class FullLoadDataToS3():
    """
    This class is used to fetch data from YouTube and save it to S3.
    """

    def __init__(self, Api_Key, Channel_Id) -> None:
        """
        Initializer for the FullLoadDataToS3 class.

        Args:
            api_key: The YouTube API key.
            channel_ids:  YouTube channel IDs in our case (T-series Channel id).
            self.s3client: it is an aws connector
        """
    ...
    def fetch_data_from_yt(self, End_Date) -> list:
        ...
        while more_pages == True:
            ...
            for i in range(len(response['items'])):
                ...
                # print(Published_date < End_Date)
                # print("\n")

                if Published_date < End_Date:
                    #It is to fetch the data range 7 days of data as defined ,
                    # or it will add the videos greater than last 7 days to list and fails if video is published prior to last 7 days
...
if __name__ == "__main__":
    ...
    #aws S3 sph-raw-data for raw data
    Bucket_name = "sph-raw-data"

    #Call the `FullLoadDataToS3_obj.run()` function with the date 7 days ago as the argument and bucket_name
    FullLoadDataToS3_obj.run(a_week , Bucket_name)